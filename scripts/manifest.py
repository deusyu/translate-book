...
def validate_for_merge(temp_dir):
    ...
    for chunk in sorted(manifest["chunks"], key=lambda c: c["order"]):
        ...
        # Check non-empty. Whitespace-only outputs also disappear during merge,
        # so treat them as invalid rather than silently dropping the chunk.
        ...
        # Check abnormally short. Below 10% of source size is more likely to be
        # a truncated/failed agent response than a valid translated book chunk.
