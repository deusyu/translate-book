...
Edit `glossary.json` between runs to fix translations; existing `glossary.json` is never overwritten — delete it to rebuild from scratch. `scripts/run_state.py` records which glossary terms each chunk used, so later glossary changes only re-translate affected chunks after the state has been recorded. Existing outputs without run state are adopted only when the current source chunk selects no glossary terms; glossary-sensitive untracked outputs are translated once before they can become resumable state.
...
Before launching subagents, `scripts/run_state.py plan <temp_dir>` decides which chunks need translation, which existing outputs only need state recording, and which are unchanged. Untracked outputs are record-only only when they do not select current glossary terms; untracked glossary-sensitive outputs are translated by default. Use `--retranslate-untracked` only when adopting an old temp dir whose remaining untracked outputs should also be forced through translation. If a run is interrupted, re-running skips chunks that already have valid output files and current state. Failed chunks are retried once automatically.
