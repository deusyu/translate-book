...
Edit `glossary.json` between runs to fix translations; existing `glossary.json` is never overwritten — delete it to rebuild from scratch. `scripts/run_state.py` records which glossary terms each chunk used, so later glossary changes only re-translate affected chunks after the state has been recorded. If existing translated chunks predate `run_state.json`, only chunks with no selected glossary terms are adopted as-is by default; untracked chunks that currently receive glossary terms are re-translated so stale terminology is not locked in.
