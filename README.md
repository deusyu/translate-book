...
| Want page-number footers stripped from PDF output | By default, standalone numeric lines are preserved unless adjacent to Calibre noise, because they may be real content. Pass `--auto-strip-page-numbers` to delete detected monotonic page-number sequences (e.g. `1, 2, 3, ...`), or `--strip-page-numbers` to aggressively delete every standalone-digit line. These flags abort if cached `input.md` or `chunk*.md` files already exist — delete them first so the option actually takes effect. |
