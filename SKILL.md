...
By default, existing outputs without `run_state.json` records are reused only
when the current glossary selects no terms for that chunk; untracked outputs
that would receive a term table are re-translated so stale terminology is not
silently adopted. If the user explicitly asks to force every untracked existing
output through the current glossary, add `--retranslate-untracked`.
