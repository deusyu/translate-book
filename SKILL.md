...
If the user explicitly asks to re-translate every output produced before
`run_state.json` existed, add `--retranslate-untracked`. By default, existing
untracked outputs are only recorded without translation when their source chunk
does not use any current glossary terms; chunks that do use glossary terms are
scheduled for translation so edited terminology is not silently skipped.
...
- `record_only_chunk_ids` — existing valid outputs that need `run_state.json`
  records but do not need translation because no current glossary terms apply.
