...
class RunStateTests(unittest.TestCase):
    ...
    def test_untracked_glossary_sensitive_output_needs_translation_by_default(self):
    ...
    def test_recording_unselected_untracked_output_leaves_glossary_sensitive_untracked(self):
        tmp, temp_dir = self._workspace()
        with tmp:
            recorded = run_state.record_chunks(str(temp_dir), ["chunk0002"])
            plan = run_state.plan(str(temp_dir))

        self.assertEqual(recorded, ["chunk0002"])
        self.assertEqual(plan["translation_chunk_ids"], ["chunk0001"])
        self.assertEqual(plan["unchanged_chunk_ids"], ["chunk0002"])

    def test_untracked_outputs_without_glossary_are_record_only_by_default(self):
        tmp, temp_dir = self._workspace()
        with tmp:
    ...
    def test_new_alias_on_already_selected_term_changes_term_hash(self):
        tmp = tempfile.TemporaryDirectory()
        with tmp:
            temp_dir = Path(tmp.name)
            self._write(temp_dir / "input.md", "Tai and Taig appear together.\n")
            self._write(temp_dir / "chunk0001.md", "Tai and Taig appear together.\n")
            self._write(temp_dir / "output_chunk0001.md", "太一和泰格一起出现。\n")
            create_manifest(str(temp_dir), ["chunk0001.md"], str(temp_dir / "input.md"))
            self._write(temp_dir / "glossary.json", json.dumps(glossary_doc(), ensure_ascii=False))
            run_state.record_chunks(str(temp_dir), ["chunk0001"])
        ...
        self.assertEqual(plan["translation_chunk_ids"], ["chunk0001"])
        self.assertEqual(plan["chunks"][0]["reasons"][0]["code"], "glossary_term_hash_changed")
