...
class RunStateTests(unittest.TestCase):
    ...
    def test_untracked_outputs_without_selected_terms_are_record_only_by_default(self):
        tmp = tempfile.TemporaryDirectory()
        with tmp:
            temp_dir = Path(tmp.name)
            self._write(temp_dir / "input.md", "No glossary hit.\n")
            self._write(temp_dir / "chunk0001.md", "No glossary hit.\n")
            self._write(temp_dir / "output_chunk0001.md", "没有术语。\n")
            create_manifest(str(temp_dir), ["chunk0001.md"], str(temp_dir / "input.md"))
            self._write(temp_dir / "glossary.json", json.dumps(glossary_doc(), ensure_ascii=False))
            plan = run_state.plan(str(temp_dir))

        self.assertEqual(plan["translation_chunk_ids"], [])
        self.assertEqual(plan["record_only_chunk_ids"], ["chunk0001"])
    ...
    def test_alias_change_for_already_selected_term_needs_translation(self):
        ...
        with tmp:
            ...
            self._write(temp_dir / "output_chunk0001.md", "太一和泰格出现。\n")
            create_manifest(str(temp_dir), ["chunk0001.md"], str(temp_dir / "input.md"))
            self._write(temp_dir / "glossary.json", json.dumps(glossary_doc(), ensure_ascii=False))
            run_state.record_chunks(str(temp_dir), ["chunk0001"])
