...
class HashTests(unittest.TestCase):
    ...
    def test_term_hash_changes_when_aliases_change(self):
        ...
        t2 = {'source': 'Tai', 'target': '太一', 'category': 'person', 'aliases': ['Taig', 'Taighi']}
        self.assertNotEqual(glossary.term_hash(t1), glossary.term_hash(t2))

    def test_term_hash_alias_order_is_stable(self):
        t1 = {'source': 'Tai', 'target': '太一', 'category': 'person', 'aliases': ['Taig', 'Taighi']}
        t2 = {'source': 'Tai', 'target': '太一', 'category': 'person', 'aliases': ['Taighi', 'Taig']}
        self.assertEqual(glossary.term_hash(t1), glossary.term_hash(t2))

    def test_term_hash_without_aliases_keeps_legacy_payload(self):
        t1 = {'source': 'Tai', 'target': '太一', 'category': 'person'}
        t2 = {'source': 'Tai', 'target': '太一', 'category': 'person', 'aliases': []}
        self.assertEqual(glossary.term_hash(t1), glossary.term_hash(t2))
