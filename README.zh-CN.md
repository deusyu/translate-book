...
可在两次运行之间编辑 `glossary.json` 修正译法。已存在的 `glossary.json` 不会被覆盖 — 删除它才会重建。`scripts/run_state.py` 会记录每个 chunk 用到的术语表状态，因此后续术语表变化只会重译受影响的 chunk（前提是该 chunk 已写入 run_state）。没有 run_state 的既有输出只有在当前源 chunk 不命中术语表时才会被采纳；命中术语表的未记录输出必须先重译一次，才能成为可续跑状态。
...
启动 subagent 前，`scripts/run_state.py plan <temp_dir>` 会判断哪些 chunk 需要翻译、哪些已有输出只需记录状态、哪些无需处理。未记录输出只有在不命中当前术语表时才会作为 record-only 采纳；命中术语表的未记录输出默认会重译。只有在接管旧 temp 目录且明确希望其余未记录输出也全部重译时，才使用 `--retranslate-untracked`。如果运行中断，重新运行会跳过已有合法输出且状态仍有效的 chunk。翻译失败的 chunk 会自动重试一次。
